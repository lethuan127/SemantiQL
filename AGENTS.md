...
`src/` holds the four layers, `tests/` mirrors them, `specs/` records changes, `docs/` explains the
system. Four top-level directories are less obvious and worth knowing before you touch them:

| Path | | |
|---|---|---|
| `plugin/`, `.claude-plugin/`, `bundle/` | **shipped product** | the plugin, the marketplace manifest that installs it, the Desktop bundle |
| `.claude/` | **this repo's own tooling** | not shipped, and not to be confused with `.claude-plugin/` — they differ by a hyphen and are opposites |
| `scripts/fixtures/` | committed **code** | fixture loaders and the run judge |
| `.test-workspace/` | ignored **output** | fixture data, answer keys, run logs. Code placed here is invisible to a clone |

**[docs/07-code-map.md](docs/07-code-map.md) is the detail** — every directory, what you are allowed to
do to each, and the four distinctions that have already cost something. Read it before your first
change; it is short, and it is the file that tells you where a change goes.
